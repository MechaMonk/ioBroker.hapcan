 ...
 */
    ...
    */
    async createLeds(node, group, device, ledIndex) {
        // this.adapter.log.info(`[Creator] [createLeds]`);
...
*/
    async createOCs(node, group, device, outputIndex) {
        const channel = { id: 'oc', name: 'Open Collectors', type: 'oc' };
        const result = await this.createDeviceChannel(node, group, device, channel);
        const iHex = tools.hex2string(outputIndex);
        await this.checkCreateState(result.states, device.id, channel.id, `${iHex}_on`,
            {
                name: `${iHex} is On`,
                type: 'boolean',
...
* @param {number} group
* @param {{id:string, name:string}} device
* @param {{index:number, name:string}} led
*/
    async createRgb(node, group, device, led) {
        const channel = { id: 'rgb', name: 'RGB LED Controller', type: 'rgb' };
        const result = await this.createDeviceChannel(node, group, device, channel);
        await this.checkCreateState(result.states, device.id, channel.id, led.name,
            {
                name: `${led.name.slice(0, 1).toUpperCase()}${led.name.slice(1)}`,
                type: 'number',
                role: 'value',
                read: true,
                write: true,
        ...
        await this.checkCreateState(result.states, device.id, channel.id, 'relay',
            {
                name: 'Any channel is ON',
                type: 'boolean',
                role: 'switch',
                read: true,
                write: false,
    ...
    async updateDeviceName(deviceId, description, append) {
        ...
        const update =
            ...
            async function (result) {
                ...
                if (device === undefined) {
                    _this.adapter.log.warn('[Creator] Device ' + fullDeviceId + ' NOT found. Cannot set device name.');
                } else {
                    // this.adapter.log.info('Device ' + fullDeviceId + ' found.');
                    if (append) {
                        // _this.adapter.log.info(`[Creator] Appending name „${description}” to „${device.common.name}”`);
                        device.common.name += description;
                    } else {
                        // _this.adapter.log.info(`[Creator] Setting name „${description}”, current = „${device.common.name}”`);
                        device.common.name = description;
                    }
