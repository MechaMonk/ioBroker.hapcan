    ...
    */
    async createDeviceChannel(node, group, device, channel) {
        ...
        const create = async function () {
            ...
            const fullDeviceId = _this._adapter.namespace + '.' + device.id;
            const fullChannelId = fullDeviceId + '.' + channel.id;

            await _this._adapter.getDevicesAsync().then(result => { found = result.some(value => value._id === fullDeviceId); });
            if (found) {
                // this.log.info('Device ' + fullDeviceId + ' found.');
                await _this._adapter.getChannelsOfAsync(device.id).then(result => { found = result.some(value => value._id === fullChannelId); });
            } else {
                //this.log.info('Device ' + fullDeviceId + ' NOT found. Creating.');
                await _this._adapter.createDeviceAsync(device.id, { name: device.name }, { node: node, group: group });
                _this._adapter.log.info('Device ' + fullDeviceId + ' created.');
            }
            if (!found) {
                //this.log.info('Channel ' + fullChannelId + ' NOT found. Creating.');
                await _this._adapter.createChannelAsync(device.id, channel.id, { name: channel.name }, { type: channel.type });
                _this._adapter.log.info('Channel ' + fullChannelId + ' created.');
            } else {
                // this.log.info('Channel ' + fullChannelId + ' found.');
                await _this._adapter.getStatesOfAsync(device.id, channel.id).then(result => states = result);
            }
...
*/
    async checkCreateState(states, deviceId, channelId, stateId, common, native) {
        const fullStateId = `${this._adapter.namespace}.${deviceId}.${channelId}.${stateId}`;
        if (!states.some(value => value._id === fullStateId)) {
            return this._adapter.createStateAsync(deviceId, channelId, stateId, common, native);
        }
    }
