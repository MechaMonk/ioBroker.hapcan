    ...
    async parseFrame(hexData) {
        ...
        if (hexData.length == 30) {
            if (hexData.substr(0, 2) != 'AA' || hexData.substr(28, 2) != 'A5') {
                this.log.warn('[Decoder] Invalid frame: ' + hexData);
                return;
            }
            ...
            this.log.warn('[Decoder] Special frame ignored: ' + hexData);
            return;
        }
        ...
        if (checksumCounted != chksum) {
            this.log.warn('[Decoder] Invalid frame, checksum validation failed. Expected: ' + chksum.toString(16).toUpperCase().padStart(2, '0') + ', actual: ' + checksumCounted.toString(16).toUpperCase().padStart(2, '0'));
            return;
        }
