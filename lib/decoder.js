...
class Decoder extends EventEmitter {
    ...
    async put(chunk) {
        ...
        //this.log.info('[Decoder] [put] chunk = ' + hexChunk);
    ...
    */
    async parseFrame(hexData) {
        ...
        if (hexData.length == 30) {
            ...
            this.log.warn('[Decoder] [parseFrame] Special frame ignored: ' + hexData);
            return;
        }
        ...
        if (checksumCounted != chksum) {
            this.log.warn('[Decoder] [parseFrame] Invalid frame, checksum validation failed. Expected: ' + chksum.toString(16).toUpperCase().padStart(2, '0') + ', actual: ' + checksumCounted.toString(16).toUpperCase().padStart(2, '0'));
            return;
        }
