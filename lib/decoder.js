...
class Decoder extends EventEmitter {
    constructor() {
        ...
        this.on('decode', this.parseFrame);
    }
    ...
    put(chunk) {
        ...
        this.tryDecode();
        // FIXME
        //this.emit('logWarn', hexChunk);
        //this.buffer = '';
    }

    tryDecode() {
        //console.log('[Decoder] [tryDecode] buffer = ' + this.buffer);
        //console.log('[Decoder] [tryDecode]');
        ...
        if (this.buffer.length == 30) {
            if (this.buffer.substr(28, 2) == 'A5') {
                this.emit('decode', this.buffer);
                //counter++;
                this.buffer = '';
            }
        }

        //console.log('[Decoder] [tryDecode] counter = ' + counter);
        //this.emit('logInfo', '[Decoder] [tryDecode] counter = ' + counter);
    }
 ...
 */
    parseFrame(hexData) {
        ...
        if (hexData.length != 30 || hexData.substr(0, 2) != 'AA' || hexData.substr(28, 2) != 'A5') {
            this.emit('logError', '[Decoder] [parseFrame] Invalid frame: ' + hexData);
            return;
        }
        // FrameType: 12 bits
        ...
        if (checksumCounted != chksum) {
            this.emit('logError', '[Decoder] [parseFrame] Invalid frame, checksum validation failed. Expected: ' + chksum.toString(16).toUpperCase().padStart(2, '0') + ', actual: ' + checksumCounted.toString(16).toUpperCase().padStart(2, '0'));
            return;
        }
        ...
        this.emit('logWarn', msg);

        this.emit('frame', frameType, flags, node, group, data);
        // FIXME
        //for (let i = 0; i < 10; i++) { this.emit('frame', frameType, flags, node, group, data); }
